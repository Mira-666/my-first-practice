...
int main(){
	...
	//定义一个区域
	SDL_Rect rect = {333,111,50,50 };

	//在Surface上进行绘制
	SDL_FillRect(surf, &rect, 0xff0000);//颜色分量用十六进制
	...
	while (true) 
	{
		...
		SDL_FillRect(surf, &rect, 0x000000);//上一个位置涂黑
		rect.x++;//横坐标往右移动   可以用自增+=
		SDL_FillRect(surf, &rect, 0x0000ff);//重新绘制矩形
